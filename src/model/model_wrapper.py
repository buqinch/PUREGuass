...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        __len , _, _, h, w = batch["target"]["image"].shape
        ...
        # print(batch["context"]["intrinsics"])
        for i in range(__len):
            result = model_Geo.calibrate(batch["context"]["image"][i], shared_intrinsics=True)
            result_cam = result["camera"].K*0
            result_cam[:, 2, 2] = 1.0
            # print(result_cam)
            batch["context"]["intrinsics"][i] = result_cam
            # print("camera:", result_cam)
