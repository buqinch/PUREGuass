...
class SwiGLU(nn.Module):
    """
    SwiGLU: gated linear unit with SiLU gating.
    proj 到 2*hidden，然后 split，一半做 value，一半做 gate，用 SiLU 激活 gate。
    """
    ...
    def forward(self, x):
        x_val, x_gate = self.proj(x).chunk(2, dim=-1)
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, bias=True, drop=0.):
        ...
        # 用 SwiGLU 代替两步：Linear→GELU→split
        self.swiglu = SwiGLU(in_features, hidden_features, bias=bias)
