...
# 更换导入，使用DIFF版本的Transformer
# from .croco.blocks import DecoderBlock
from .croco.blocks_DIFF import DIFFDecoderBlock
from .croco.croco_DIFF import CroCoNet
...
class BackboneCrocoCfg:
    name: Literal["croco", "croco_multi","croco_DIFF"]
    ...
    lambda_init: float = 0.1  # 添加DIFF特有参数
    

    


class AsymmetricCroCoDIFF(CroCoNet):
    ...
    def __init__(self, cfg: BackboneCrocoCfg, d_in: int) -> None:
        ...
        # 添加lambda_init到croco_args
        self.lambda_init = cfg.lambda_init
        croco_param_copy = deepcopy(croco_params[cfg.model])
        croco_param_copy['lambda_init'] = self.lambda_init
        self.croco_args = fill_default_args(croco_param_copy, CroCoNet.__init__)

        super().__init__(**croco_param_copy)
        ...
        # 添加扩散模型组件
        # self.use_diffusion = cfg.use_diffusion
        # if self.use_diffusion:
        #     self.feature_diffusion = LDMFeatureDiffusionModule(
        #         feature_dim=self.enc_embed_dim,
        #         pos_dim=self.rope.dim if hasattr(self, 'rope') else 256,
        #         diffusion_steps=cfg.diffusion_steps,
        #         pretrained_model_id=cfg.ldm_model_id,
        #         scheduler_type=cfg.scheduler_type,
        #         beta_start=cfg.beta_start,
        #         beta_end=cfg.beta_end,
        #     )
        #     self.diffusion_noise_level = cfg.diffusion_noise_level
        #     self.use_clip_conditioning = cfg.use_clip_conditioning
    ...
    def _set_decoder(self, enc_embed_dim, dec_embed_dim, dec_num_heads, dec_depth, mlp_ratio, norm_layer, norm_im2_in_dec,lambda_init):
        ...
        # 使用DIFFDecoderBlock替代DecoderBlock，并添加DIFF特有参数
        self.dec_blocks = nn.ModuleList([
            DIFFDecoderBlock(
                dec_embed_dim, dec_num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, 
                norm_layer=norm_layer, norm_mem=norm_im2_in_dec, rope=self.rope,
                lambda_init=self.lambda_init, layer_idx=i, max_layer=dec_depth
            )
    ...
    def forward(self,
        ...
        #     # 准备CLIP特征作为条件
        #     clip_embedding = None
        #     if self.use_clip_conditioning and "clip_features" in context:
        #         clip_embedding = context["clip_features"]
            
        #     # 处理视图1的特征和位置编码
        #     feat1, pos1, diff_output1 = self.feature_diffusion.process_features_and_pos(
        #         feat1, pos1, 
        #         clip_embedding[:, 0] if clip_embedding is not None else None, 
        #         noise_level=self.diffusion_noise_level,
        #         guidance_scale=self.guidance_scale,
        #         steps=min(20, self.feature_diffusion.diffusion_steps)  # 使用较少步数提高速度
        #     )
            
        #     # 处理视图2的特征和位置编码
        #     feat2, pos2, diff_output2 = self.feature_diffusion.process_features_and_pos(
        #         feat2, pos2, 
        #         clip_embedding[:, 1] if clip_embedding is not None else None,
        #         noise_level=self.diffusion_noise_level,
        #         guidance_scale=self.guidance_scale,
        #         steps=min(20, self.feature_diffusion.diffusion_steps)
        #     )
