...
# from .diffusion_base import GGDM,FeatureGGDM

inf = float('inf')
device = "cuda" if torch.cuda.is_available() else "cpu"
model_Geo = GeoCalib().to(device)
croco_params = {
...
#         self.freeze_expect_diffusion_1 = True
        
#         if self.freeze_expect_diffusion_1:
#             self.freeze_except_diffusion()
        
        
        # self.set_freeze(freeze)
        
#     def freeze_except_diffusion(self):
#         """冻结除扩散模型外的所有参数"""
#         # 首先冻结所有参数
...
#         print(f"已冻结参数: {frozen_count:,} ({frozen_count/(frozen_count+trainable_count):.2%})")
#         print(f"可训练参数: {trainable_count:,} ({trainable_count/(frozen_count+trainable_count):.2%})")
    ...
    def _encode_image(self, image, true_shape, intrinsics_embed=None):
        ...
        # print(123)
        # print(image.shape)
        # result = model_Geo.calibrate(image, shared_intrinsics=True)
        # print(result['camera'].shape)
        # print(intrinsics_embed.shape) #  
